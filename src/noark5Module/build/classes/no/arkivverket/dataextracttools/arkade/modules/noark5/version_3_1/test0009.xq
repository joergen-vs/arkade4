...
(:
 ...
 : Type: Analyse
 : Opptelling av antall forekomster av klasse uten underklasser eller mapper i det primære klassifikasjonssystemet i arkivstruktur.xml.
...
(: Primært klassifikasjonssystem skal være det eneste ene klassifikasjonssystemet
   under en arkivdel som inneholder klasser med mapper.
 :)
...
<activity name="{$testName}"
  ...
  {  
  if (exists($testDescription)) then ( 
  <description>{$testDescription}</description>
  ) else ()
  }
  <result>
    {
    if (exists($resultDescription)) then (
              ...
              <resultItem name="klassifikasjonssystemerMedMapper" type="{if (count($pk) = 1)
                ...
                <resultItems>
                  ...
                  <resultItem name="klassifikasjonssystem" type="info">
                    ...
                    <label>Antall klasser uten underklasser eller mapper</label>
                    <content>
{count($arkivdel//n5a:klassifikasjonssystem[n5a:systemID=data($ks)]//n5a:klasse[not(n5a:registrering or n5a:mappe or n5a:klasse)])}       
                    </content>
                  </resultItem>
                  }
                </resultItems>
          ...
          <resultItem name="overordnetResultat">
            <resultItems>
              <resultItem type="info">
                <label>Arkivdelen mangler primært klassifikasjonssystem</label>
              </resultItem>
            </resultItems>
          </resultItem>
          )}
